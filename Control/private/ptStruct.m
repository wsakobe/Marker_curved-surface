% 基于点列表"ptList"及其连接关系"ledge"组织多个网格
% Organize grids based on point list "ptList" and ledge information "ledge"

% - array
%   Nx1的cell, 每个cell对应一个连通的grid.
%   grid以矩阵形式保存点的网格连接关系, 其中元素为点在"ptList"中的索引, 不完整位置以NaN表示.
...
function [array,E] = ptStruct(img,ptList,ledge)
    ...
    %% 三角剖分, 移除斜边, 拼合为矩形(矩形边应贴合其中一个顶点的跳白线和另一个顶点的跳黑线)
